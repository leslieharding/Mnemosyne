...
# Reference to the Dionysus card collection
var dionysus_collection: GodCardCollection
...
func _ready():
	print("=== DIONYSUS SCENE STARTING ===")
	
	# Load the Dionysus card collection
	dionysus_collection = load("res://Resources/Collections/Dionysus.tres")
	
	if not dionysus_collection:
		print("ERROR: Failed to load Dionysus collection!")
		# Try alternative loading method
		var collection_path = "res://Resources/Collections/Dionysus.tres"
		if ResourceLoader.exists(collection_path):
			dionysus_collection = ResourceLoader.load(collection_path)
			if dionysus_collection:
				print("Dionysus collection loaded with ResourceLoader")
			else:
				print("ResourceLoader also failed")
				return
		else:
			print("Dionysus.tres file does not exist!")
			return
	
	print("Dionysus collection loaded successfully")
	print("Cards: ", dionysus_collection.cards.size())
	print("Decks: ", dionysus_collection.decks.size())
...
func _on_start_game_button_pressed() -> void:
	if selected_deck_index >= 0:
		...
		var deck_def = dionysus_collection.decks[selected_deck_index]
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		var god_progress = progress_tracker.get_god_progress("Dionysus")
		if not deck_def.is_unlocked("Dionysus", god_progress):
		...
		if has_node("/root/OptionalBattleTrackerAutoload"):
			get_node("/root/OptionalBattleTrackerAutoload").start_new_run("Dionysus", deck_def.deck_name)
...
func setup_deck_buttons():
	...
	if not dionysus_collection:
		print("ERROR: Dionysus collection is null in setup_deck_buttons!")
		return
	
	if dionysus_collection.decks.size() == 0:
		print("ERROR: Dionysus collection has no decks!")
		return
	...
	print("Dionysus collection has ", dionysus_collection.decks.size(), " decks")
	...
	if progress_tracker:
		...
		god_progress = progress_tracker.get_god_progress("Dionysus")
	...
	for i in range(deck_buttons.size()):
		...
		if i < dionysus_collection.decks.size():
			# We have a deck for this button
			var deck_def = dionysus_collection.decks[i]
			...
			# Check unlock status
			var is_unlocked = deck_def.is_unlocked("Dionysus", god_progress)
...
func select_deck(index: int) -> void:
	var deck_def = dionysus_collection.decks[index]
	var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
	var god_progress = progress_tracker.get_god_progress("Dionysus")
	var is_unlocked = deck_def.is_unlocked("Dionysus", god_progress)
	...
	for i in range(dionysus_collection.decks.size()):  # Only loop through actual decks
		...
		var deck = dionysus_collection.decks[i]
		if deck.is_unlocked("Dionysus", god_progress):
			button.disabled = false
			button.modulate = Color.WHITE
	...
	print("Selected deck: ", dionysus_collection.decks[index].deck_name)
	if is_unlocked:
		print("Description: ", dionysus_collection.decks[index].deck_description)
	else:
...
func display_unlock_requirements(deck_index: int) -> void:
	if not dionysus_collection or deck_index < 0 or deck_index >= dionysus_collection.decks.size():
		return
	
	var deck_def = dionysus_collection.decks[deck_index]
...
func display_deck_cards(deck_index: int) -> void:
	...
	for i in range(deck_def.card_indices.size()):
		...
		if card_index < dionysus_collection.cards.size():
			var card = dionysus_collection.cards[card_index]
...
func create_deck_card_display(card: CardResource, card_index: int) -> Control:
	...
	if has_node("/root/GlobalProgressTrackerAutoload"):
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		current_level = progress_tracker.get_card_level("Dionysus", card_index)
	...
	if has_node("/root/GlobalProgressTrackerAutoload"):
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		var exp_data = progress_tracker.get_card_total_experience("Dionysus", card_index)
