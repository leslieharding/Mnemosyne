...
var card_displays: Array = []  # Remove CardDisplay type hint
...
func create_experience_info_display(card_index: int, tracker) -> Control:
	...
	if not exp_container:
		print("Error: Failed to create VBoxContainer")
	...
	if not title_label:
		print("Error: Failed to create title label")
	...
	if tracker and is_instance_valid(tracker):
		...
		# Null check the exp_data
		if not exp_data:
			print("Error: exp_data is null for card_index: ", card_index)
			exp_data = {"capture_exp": 0, "defense_exp": 0}
		
		# Capture experience label with null checks
		var capture_label = Label.new()
		if capture_label:
			var capture_exp = exp_data.get("capture_exp", 0)
			...
			print("Error: Failed to create capture label")
		
		# Defense experience label with null checks
		var defense_label = Label.new()
		if defense_label:
			var defense_exp = exp_data.get("defense_exp", 0)
			...
			print("Error: Failed to create defense label")
		
		# Add total for this run with null checks
		var capture_exp = exp_data.get("capture_exp", 0)
		var defense_exp = exp_data.get("defense_exp", 0)
		var total_run_exp = capture_exp + defense_exp
		if total_run_exp > 0:
			...
			if total_label:
				...
				print("Error: Failed to create total label")
	else:
		# No tracker available - with null checks
		...
		if no_data_label:
			...
			print("Error: Failed to create no_data label")
