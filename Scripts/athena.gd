...
# Reference to the Athena card collection
var athena_collection: GodCardCollection
...
func _ready():
	print("=== ATHENA SCENE STARTING ===")
	
	# Load the Athena card collection
	athena_collection = load("res://Resources/Collections/Athena.tres")
	
	if not athena_collection:
		print("ERROR: Failed to load Athena collection!")
		# Try alternative loading method
		var collection_path = "res://Resources/Collections/Athena.tres"
		if ResourceLoader.exists(collection_path):
			athena_collection = ResourceLoader.load(collection_path)
			if athena_collection:
				print("Athena collection loaded with ResourceLoader")
			else:
				print("ResourceLoader also failed")
				return
		else:
			print("Athena.tres file does not exist!")
			return
	
	print("Athena collection loaded successfully")
	print("Cards: ", athena_collection.cards.size())
	print("Decks: ", athena_collection.decks.size())
...
func _on_start_game_button_pressed() -> void:
	if selected_deck_index >= 0:
		...
		var deck_def = athena_collection.decks[selected_deck_index]
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		var god_progress = progress_tracker.get_god_progress("Athena")
		if not deck_def.is_unlocked("Athena", god_progress):
		...
		if has_node("/root/OptionalBattleTrackerAutoload"):
			get_node("/root/OptionalBattleTrackerAutoload").start_new_run("Athena", deck_def.deck_name)
...
func setup_deck_buttons():
	...
	if not athena_collection:
		print("ERROR: Athena collection is null in setup_deck_buttons!")
		return
	
	if athena_collection.decks.size() == 0:
		print("ERROR: Athena collection has no decks!")
		return
	...
	print("Athena collection has ", athena_collection.decks.size(), " decks")
	...
	if progress_tracker:
		...
		god_progress = progress_tracker.get_god_progress("Athena")
	...
	for i in range(deck_buttons.size()):
		...
		if i < athena_collection.decks.size():
			# We have a deck for this button
			var deck_def = athena_collection.decks[i]
			...
			# Check unlock status
			var is_unlocked = deck_def.is_unlocked("Athena", god_progress)
...
func select_deck(index: int) -> void:
	var deck_def = athena_collection.decks[index]
	var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
	var god_progress = progress_tracker.get_god_progress("Athena")
	var is_unlocked = deck_def.is_unlocked("Athena", god_progress)
	...
	for i in range(athena_collection.decks.size()):  # Only loop through actual decks
		...
		var deck = athena_collection.decks[i]
		if deck.is_unlocked("Athena", god_progress):
			button.disabled = false
			button.modulate = Color.WHITE
	...
	print("Selected deck: ", athena_collection.decks[index].deck_name)
	if is_unlocked:
		print("Description: ", athena_collection.decks[index].deck_description)
	else:
...
func display_unlock_requirements(deck_index: int) -> void:
	if not athena_collection or deck_index < 0 or deck_index >= athena_collection.decks.size():
		return
	
	var deck_def = athena_collection.decks[deck_index]
...
func display_deck_cards(deck_index: int) -> void:
	...
	for i in range(deck_def.card_indices.size()):
		...
		if card_index < athena_collection.cards.size():
			var card = athena_collection.cards[card_index]
...
func create_deck_card_display(card: CardResource, card_index: int) -> Control:
	...
	if has_node("/root/GlobalProgressTrackerAutoload"):
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		current_level = progress_tracker.get_card_level("Athena", card_index)
	...
	if has_node("/root/GlobalProgressTrackerAutoload"):
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		var exp_data = progress_tracker.get_card_total_experience("Athena", card_index)
