...
# Reference to the Aphrodite card collection
var aphrodite_collection: GodCardCollection
...
func _ready():
	print("=== APHRODITE SCENE STARTING ===")
	
	# Load the Aphrodite card collection
	aphrodite_collection = load("res://Resources/Collections/Aphrodite.tres")
	
	if not aphrodite_collection:
		print("ERROR: Failed to load Aphrodite collection!")
		# Try alternative loading method
		var collection_path = "res://Resources/Collections/Aphrodite.tres"
		if ResourceLoader.exists(collection_path):
			aphrodite_collection = ResourceLoader.load(collection_path)
			if aphrodite_collection:
				print("Aphrodite collection loaded with ResourceLoader")
			else:
				print("ResourceLoader also failed")
				return
		else:
			print("Aphrodite.tres file does not exist!")
			return
	
	print("Aphrodite collection loaded successfully")
	print("Cards: ", aphrodite_collection.cards.size())
	print("Decks: ", aphrodite_collection.decks.size())
...
func _on_start_game_button_pressed() -> void:
	if selected_deck_index >= 0:
		...
		var deck_def = aphrodite_collection.decks[selected_deck_index]
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		var god_progress = progress_tracker.get_god_progress("Aphrodite")
		if not deck_def.is_unlocked("Aphrodite", god_progress):
		...
		if has_node("/root/OptionalBattleTrackerAutoload"):
			get_node("/root/OptionalBattleTrackerAutoload").start_new_run("Aphrodite", deck_def.deck_name)
...
func setup_journal_button():
	if not journal_button:
		...
		print("Aphrodite: Journal button added with CanvasLayer")
...
func setup_deck_buttons():
	...
	if not aphrodite_collection:
		print("ERROR: Aphrodite collection is null in setup_deck_buttons!")
		return
	
	if aphrodite_collection.decks.size() == 0:
		print("ERROR: Aphrodite collection has no decks!")
		return
	...
	print("Aphrodite collection has ", aphrodite_collection.decks.size(), " decks")
	...
	if progress_tracker:
		...
		god_progress = progress_tracker.get_god_progress("Aphrodite")
	...
	for i in range(deck_buttons.size()):
		...
		if i < aphrodite_collection.decks.size():
			# We have a deck for this button
			var deck_def = aphrodite_collection.decks[i]
			...
			# Check unlock status
			var is_unlocked = deck_def.is_unlocked("Aphrodite", god_progress)
...
func select_deck(index: int) -> void:
	var deck_def = aphrodite_collection.decks[index]
	var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
	var god_progress = progress_tracker.get_god_progress("Aphrodite")
	var is_unlocked = deck_def.is_unlocked("Aphrodite", god_progress)
	...
	for i in range(aphrodite_collection.decks.size()):  # Only loop through actual decks
		...
		var deck = aphrodite_collection.decks[i]
		if deck.is_unlocked("Aphrodite", god_progress):
			button.disabled = false
			button.modulate = Color.WHITE
	...
	print("Selected deck: ", aphrodite_collection.decks[index].deck_name)
	if is_unlocked:
		print("Description: ", aphrodite_collection.decks[index].deck_description)
	else:
...
func display_unlock_requirements(deck_index: int) -> void:
	if not aphrodite_collection or deck_index < 0 or deck_index >= aphrodite_collection.decks.size():
		return
	
	var deck_def = aphrodite_collection.decks[deck_index]
...
func display_deck_cards(deck_index: int) -> void:
	...
	for i in range(deck_def.card_indices.size()):
		...
		if card_index < aphrodite_collection.cards.size():
			var card = aphrodite_collection.cards[card_index]
...
func create_deck_card_display(card: CardResource, card_index: int) -> Control:
	...
	if has_node("/root/GlobalProgressTrackerAutoload"):
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		current_level = progress_tracker.get_card_level("Aphrodite", card_index)
	...
	if has_node("/root/GlobalProgressTrackerAutoload"):
		var progress_tracker = get_node("/root/GlobalProgressTrackerAutoload")
		var exp_data = progress_tracker.get_card_total_experience("Aphrodite", card_index)
